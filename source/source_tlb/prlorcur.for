...
C
C          FUNCTION:-
C               It reads the output of LORCUR and types out a convenient
...
*  PURPOSE
*        To type and make a .LIS file of the LORCUR output file.
*
*  METHOD
...
C
      ENDDO
C
C  Type and store  Profiles Heading
C
      IF (KTYPE.EQ.1) THEN
         CALL WRUSER(' ',ISTAT)
         CALL WRUSER(' ',ISTAT)
         WRITE(PRBUF,903)
  903    FORMAT(' ',' PROFILES ARE')
         CALL WRUSER(PRBUF,ISTAT)
      ENDIF
      WRITE(LU,904)
  904 FORMAT(' ',//,' ','PROFILES  ')
C
C  Type and store profiles
C
      DO J = 1,LSTLEN
C
C COPY ENTRY INTO BUFFER TO ALLOW DIFFERENT DATA TYPES
...
C
         ...
         ENDDO
C
C  Type out and store profile
C
         DO L = 12,16
            IF (DATA(L).GT.9999.0) DATA(L) = 9999.0
            IF (DATA(L).LT.-999.0) DATA(L) = -999.0
         ENDDO
         IF (KTYPE.EQ.1) THEN
            WRITE (PRBUF,931) (ID(I:I),I=1,20),(DATA(L),L=12,16)
  931       FORMAT(' ',20A1,5X,5F8.3)
            CALL WRUSER(PRBUF,ISTAT)
         ENDIF
C
         WRITE (LU,911) (ID(I:I),I=1,20),(DATA(L),L=12,16)
  932    FORMAT(' ',20A1,5F8.3)
      ENDDO
C
C ---------------------------------------------------
C
