...
C
      IF(BORDER.GE.0.05)THEN
        ...
        CALL RDKEYC('LABEL',.FALSE.,1,LABEL,NVAL,IERR)
        IEL=80
        DO WHILE (LABEL(IEL:IEL).EQ.' ')
          IEL=IEL-1
        END DO
C
C     - make a zone 1cm high and 19cm long, immediately below the image
C
        CALL SGS_ZONE(0.,1.,0.,BORDER,IZONE2,IERR)
        CALL SGS_ZSIZE(0.19,0.01,'TC',IZONE3,IERR)
        CALL SGS_SW(0.,19.,0.,1.,IERR)
C
C     - plot the label centrally in this zone, in characters 2.5mm high
C
        CALL SGS_SHTX(0.25)
        CALL SGS_STXJ('BC')
        CALL SGS_BTEXT(9.5,0.5)
