...
C
C          CALLING SEQUENCE:-
C               LINCONU [input] [output]
C
C
C          FUNCTION:-
C               It reads a Starlink image (as  unsigned 16  bit  integers)
C               together with  the  descriptor  items BSCALE and BZERO, and 
C               converts the data into REAL values using 
C               OUTPUT=INPUTxBSCALE+BZERO
C
C
C          USE:-
...
C
C         The modifications for unsigned integers were written by 
C         P M Allan                Manchester                     05-MAR-86
C
C--------------------------------------------------------------------------
...
C
      ...
      DO I=1,N
         ...
         ELSE
            OUT(I) = (REAL(IN(I)) + 65536.) * BSCALE + BZERO
         ENDIF
      END DO
      END
