...
C
C               In this variant the process may be repeated at will.
C
C
C          USE:-
C               Obviously - to look at sections in any direction through an
C               an image. It can be used for exploring the data values
C               present.
C
C
C
...
C
C
C         AGAIN          YES                  This asks if another slice is
C                                             required.
C
...
C
     ...
     :            AXIN(2),LINE)
              ...
              ENDIF
C
C         Now ask if another slice is needed.
C
              CALL CNPAR('AGAIN',ISTAT)
              FLAG=.TRUE.
              CALL RDKEYL('AGAIN',.TRUE.,1,FLAG,I,ISTAT)
              IF (ISTAT.GT.ERR_PARNUL) GO TO 999
              IF (FLAG) GO TO 100
          ENDIF
      ENDIF
C
