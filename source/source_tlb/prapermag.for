...
C
C          FUNCTION:-
C               It reads the output of APERMAG and types out a convenient
...
C
C       OUTPUT           PRAPERMAG.LIS        This is the formatted file to
C                                             which    the    results   are
C                                             written.  The  user  supplies
C                                             the  name  only;  the program
C                                             adds .LIS.
C                                             PRAPERMAG is the default
C                                             name.
C
...
*  PURPOSE
*        To type and make a .LIS file of the APERMAG output file.
*
*  METHOD
...
C
      IF (KTYPE.EQ.1) THEN
         CALL WRUSER(' ',ISTAT)
         WRITE(PRBUF,900)
  900    FORMAT(' ',' STAR','   MAG  ','      FLUX ','  DX  ',
     +       '  DY  ',' RMS ','INVAL','    BASE  ','  PIX ')
...
C
  ...
  910    FORMAT (1H ,I5,F7.2,F11.1,2F6.1,F6.1,I5,F10.1,I5)
         CALL WRUSER(PRBUF,ISTAT)
      ENDIF
C
         WRITE (LU,911) (ID(I:I),I=1,20),AX,AY,AMAG,AH,ADX,ADY,
     +                    ARMS,NINVAL,BASE,KPIX
  911 FORMAT (1H ,20A1,2F10.2,F7.2,F11.1,2F6.1,F6.1,I5,
     +        F10.1,I5)
C
C
      ENDDO
