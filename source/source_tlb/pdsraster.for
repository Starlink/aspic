...
C
C      FUNCTION:-
C       This reads a nine track PDS tape of rasters written in the
C       Scansalot format from the Forth system.
C       The rasters are written into Starlink .BDF images, as Integer
C       I*2 images, with Title, Bscale and Bzero.
C
C
C       USER PARAMETERS
C
...
C                                              the wanted image.
C                                              This is 1 greater than the
C                                              NUMFILE of the preceeding
C                                              image.
C
...
C                                              are YES,NO.
C
C        XYSIZE        Square scan assumed     Size of the Scan in X,Y
C
C        OUTPUT                                Name of file image
...
C
         ...
         NLINE = NLINE + 1
C
C  Type scan information if at start of a scan
C
         IF (LOOP) THEN
            IF (NLINE.EQ.1) THEN
               CALL WRUSER(' ',ISTAT)
               CALL WRUSER('SCAN INFORMATION',ISTAT)
  ...
  901          FORMAT(' ',40A1,I6,1X,2X,2I7,1X,2X,I5)
         ...
         ENDIF
C
C  If 1st line of scan, open Output file
C
         IF (LOOP.AND.NLINE.EQ.1) THEN
            IF (NDS.LT.1) THEN
               CALL WRUSER('NO X PIXELS. BAD FILE OR END OF FILES',
     +                     ISTAT)
               LOOP = .FALSE.
            ENDIF
C
            IF (LOOP) THEN
               CALL WRUSER('INPUT FILE PARAMS',ISTAT)
               K = 1
...
C
            IF (LOOP) THEN
               NGETS(1) = NDS
               NGETS(2) = NGETS(1)
               CALL RDKEYI('XYSIZE',.TRUE.,2,NGETS,NVAL,IERR)
               NXMAIN = NGETS(1)
               NYMAIN = NGETS(2)
               CALL CNPAR('XYSIZE',ISTAT)
               IF (NXMAIN.EQ.0.OR.NYMAIN.EQ.0) THEN
                  CALL WRUSER('BAD DIMENSIONS,ABORTED',ISTAT)
                  LOOP = .FALSE.
               ENDIF
            ENDIF
         ENDIF
...
C
         IF (LOOP.AND.NLINE.EQ.1) THEN
            CALL WRUSER(' GIVE NAME OF OUTPUT IMAGE FILE',ISTAT)
...
C
         ...
         ENDIF
C
C  Copy line into Output image
C
         IF (LOOP) THEN
            CALL DATPUT(NDENS,16384,NXMAIN,
     +                  %VAL(IPMAIN),NXMAIN,NYMAIN,NLINE)
         ENDIF
C
C  Check if reached end of segments and read last file mark
C
         IF (LOOP) THEN
            IF(NLINE.EQ.NYMAIN) THEN
               CALL PDSNIN(ID,NDS,NPOS,NDENS,N,MTRN)
               IF (NDS.EQ.0) THEN
                  NFILE = NFILE + 1
                  CALL WRUSER('READ IMAGE OK',ISTAT)
                  LOOP = .FALSE.
               ELSE
                  CALL WRUSER('END FILE MARK MISSING',ISTAT)
                  LOOP = .FALSE.
               ENDIF
            ENDIF
         ENDIF
