...
$LBL02:
$	IF F$SEARCH("USOURCE:UTILITIES.TLB").EQS."" THEN DEFINE USOURCE 'SOURCE_SOURCE'
$	IF F$SEARCH("USOURCE:UTILITIES.TLB").NES."" THEN GOTO LBL03
$	   GOSUB SOURCE_SYSTEM
$	   GOTO LBL02
$LBL03:
...
$	   COPY 'SOURCE_SOURCE'TEMPLATE.FOR SOURCE:TEMPLATE.FOR
$	   COPY 'SOURCE_SOURCE'TEMPLATE.CON SOURCE:TEMPLATE.CON
$	   COPY 'SOURCE_BUILD'TEMPLATE.BLD BUILD:TEMPLATE.BLD
$	   COPY 'SOURCE_SOURCE'TEMPLATE.HLP SOURCE:TEMPLATE.HLP
...
$!----------------------------------------------------------------------
$!
$!  LOCAL SUBROUTINE SOURCE_SYSTEM
$!
$! If the system being developed have essential files missing, then
$! this routine is called to find a directory of some other system
$! from which the files can be used (not copied, just executed).
$!
$SOURCE_SYSTEM:
$
$	IF F$TYPE(DEVSYS).NES."" THEN SOURCE_DIR=F$PARSE(DEVSYS-"@",,,"DEVICE")+F$PARSE(DEVSYS-"@",,,"DIRECTORY")-".PROCS]"
$LBL6:
$	SOURCE_TOP=F$PARSE(SOURCE_DIR+"]")-".;"
$	SOURCE_PROCS=F$PARSE(SOURCE_DIR+".PROCS]")-".;"
$	SOURCE_SOURCE=F$PARSE(SOURCE_DIR+".SOURCE]")-".;"
$	SOURCE_OBJECT=F$PARSE(SOURCE_DIR+".OBJECT]")-".;"
$	SOURCE_BUILD=F$PARSE(SOURCE_DIR+".BUILD]")-".;"
$	IF SOURCE_TOP.EQS."" THEN GOTO LBL7
$	IF SOURCE_PROCS.EQS."" THEN GOTO LBL7
$	IF SOURCE_SOURCE.EQS."" THEN GOTO LBL7
$	IF SOURCE_OBJECT.EQS."" THEN GOTO LBL7
$	IF SOURCE_BUILD.EQS."" THEN GOTO LBL7
$	RETURN
$LBL7:
$	WRITE SYS$OUTPUT "*** FILES REQUIRED WHICH DONT EXIST IN SPECIFIED SYSTEM"
$	INQUIRE SYSTEM "Give name of system from which files are to be used?"
$	SOURCE_DIR=F$PARSE(SYSTEM)-"].;"
$	GOTO LBL6
