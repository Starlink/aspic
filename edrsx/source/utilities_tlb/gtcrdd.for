      subroutine gtcrdd(name,band,frmid,iptr,ierr)
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
*PURPOSE
*       Get a pointer to an input CRDD image and copy the descriptor
*       values into common block /DESCR/ (see module DS_COM),
*       returning a pointer to the 'level' at which the frame
*       descriptors are stored in /DESCR/. The waveband of the data
*       in the file is extracted from descriptor 'CONTENT' and returned.
*
*
*SOURCE
*       GTCRDD.FOR in UTILITIES.TLB
*
*METHOD
*       Call EDRS routine GT2DIR to aquire the image and deal with any
*       errors which occur. Then extract all descriptors and store them
*       in common (see DS_COM include module).
*
*ARGUMENTS
*
*   INPUTS:
...
*       iptr    integer         Contains the address of the start of
*                               the input data.
...
*
      ...
      if(ierr.eq.0) then
*
* MAP THE 2D IMAGE DATA, AND GET THE IMAGE SIZE DESCRIPTORS
*
         call gt2dir(name,104,.false.,DS_nys(frmid),
     :               DS_nde(frmid),iptr,ierr)
         if(ierr.ne.0) then
            call wrerr('TOOBAD')
            ierr=16
         endif
...
*
* GET EXPECTED BORESIGHT CROSSING TIME AS A TIME OFFSET RELATIVE TO THE
* UTC OF THE FIRST RAW DATA SAMPLE (not used in DSB's progs)
*
*           DS_uex(frmid)=sngl(dutcx-DS_ust(frmid))
*
...
*
            call gtdscr(name,'BUNIT','CHARACTER',ival,rval,ctemp,ierr)
            if(ierr.ne.0) then
               call ptderr('BUNIT')
               ierr=10
               goto 999
            endif
            DS_bru(frmid)=ctemp(2:9)
*
