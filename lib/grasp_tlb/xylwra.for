...
C
C  PURPOSE
C      To take an array of parameters and to load them into an
C      EDRS format XY list, placing identifiers in the format #1
C      to #n, with headers for the parameters, and store the list 
C      in a file.
C
C  METHOD
...
C
      ...
      CHARACTER*(*) HEADER(NUMVAL)
      CHARACTER TITLE(1)*30,PRBUF*40,CVAL
      LOGICAL VALID
C
C
C
...
C
     ...
     +	       IERR)
         DO K = 1,NUMVAL
            CALL PTHEAD('XYWOUT',K,HEADER(K),IERR)
         ENDDO
C
C
C
