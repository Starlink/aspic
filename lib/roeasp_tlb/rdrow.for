      SUBROUTINE RDROW (ROW,NROWS,NCOLS,TAB)

C+
C    RDROW
C
C	called from EDITLEVS or EDITTABS, special version of READROW
...
C*  number mistyped
          ...
          ENDIF
        ENDDO
        IF (TAB(ROW,2).LT.0.0) THEN
C*  specific test for calibration package on INTENSITY
          CALL WRERR('MISINTEN')
          GOTO 100
        ENDIF
      ENDIF
