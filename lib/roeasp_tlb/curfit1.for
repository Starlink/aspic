...
*
      ...
      IF(IST.EQ.0) THEN
*
*       initialise the ARGS without reseting it
*
         CALL SRINIT(0,.FALSE.,IFAIL)
*
*       check for failure to set up ARGS
*
         IF(IFAIL.EQ.0)THEN
     ...
     :                  %VAL(NPXM),%VAL(NPYM),%VAL(NPSRS),
     :                  %VAL(NPSUM),%VAL(NPNSUM))
         ELSE
*
*          here when error allocating the ARGS
*
