      SUBROUTINE PLOTSTH (THRLD,WXMIN,WXMAX,WYMIN,WYMAX,
     :         INXEXT,INYEXT,INPICT,TZEROD,TMAXD,
     :         VXMIN,VXMAX,VYMIN,VYMAX,WKXEXT,WKYEXT,WKPICT,ISTATUS)

C+
C    PLOTSTH
C
C    generalised plotting subroutine to threshold and plot an image on the args
C    (the same as PLOTS except for thresholding).
...
C-
      ...
      REAL TZEROD,TMAXD,INPICT(INXEXT,INYEXT),THRLD
...
C*  value of pixel to compare with THRLD
      REAL VALUE
      INTEGER ARGSDIM
...
C*  shrink x and y 2 by 2 neighbourhood
           VALUE=(INPICT(IWX,IWY)+INPICT(IWX+1,IWY)
     :                   +INPICT(IWX,IWY+1)+INPICT(IWX+1,IWY+1))/4.0
         ELSE
C*  shrink y only
           VALUE=(INPICT(IWX,IWY)+INPICT(IWX,IWY+1))/2.0
         ENDIF
        ELSE
         IF (XF.GT.1.0) THEN
C*  shrink x only
           VALUE=(INPICT(IWX,IWY)+INPICT(IWX+1,IWY))/2.0
         ELSE
C*  no shrinking at all
           VALUE=INPICT(IWX,IWY)
         ENDIF
        ENDIF
        IF (VALUE.GT.THRLD) THEN
          WKPICT(VX,VY)=SCALE(VALUE)
        ELSE
          WKPICT(VX,VY)=0
        ENDIF
        WX=WX+XF
